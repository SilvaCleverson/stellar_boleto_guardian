...
**Última atualização:** 19/05/2026 (v1.17)
...
### D-020 · Integração Asaas multi-tenant (`Integracao/ASAAS/`)

**Data:** 19/05/2026  
**Decisão:** Integrar o gateway **Asaas** para registro automático na Stellar ao criar cobrança **BOLETO** (`PAYMENT_CREATED`), com **multi-tenant** via `TENANTS_JSON` e proxies Vercel em `api/webhooks/asaas/[tenantId]` e `api/admin/tenants`. Código em **`Integracao/ASAAS/`**; API Key Asaas **somente no servidor**.  
**Por quê:** Permite que emissores que já usam Asaas registrem boletos na blockchain sem alterar o ERP, alinhado a **D-002** (uma conta Stellar por emissor).
